...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Move()
    {
        ...
        else if(playerBody.velocity.magnitude < threshold)
        ...
        float xVelocity = Mathf.Clamp(playerBody.velocity.x, -maxSpeed, maxSpeed);
        float zVelocity = Mathf.Clamp(playerBody.velocity.z, -maxSpeed, maxSpeed);
        playerBody.velocity = new Vector3(xVelocity, playerBody.velocity.y, zVelocity); 
    ...
    void CounterMovement()
    {
        ...
        //when there is no input or the player's input is in the opposite direction slow down the player's velocity according to the slow down factor, unless the player is airborne
        if ((lateralInput == 0 && grounded) || (localVelocity.x > 0 && lateralInput < 0) || (localVelocity.x < 0 && lateralInput > 0))
            playerBody.AddForce(-localVelocity.x * transform.right * slowDownFactor * Time.fixedDeltaTime, ForceMode.Acceleration);

        if ((forwardInput == 0 && grounded) || (localVelocity.z > 0 && forwardInput < 0) || (localVelocity.z < 0 && forwardInput > 0))
            playerBody.AddForce(-localVelocity.z * transform.forward * slowDownFactor * Time.fixedDeltaTime, ForceMode.Acceleration);
    }
    ...
    void CheckSlope()
    {
        ...
        if (Physics.Raycast(frontCheck, out slopeCheck, .5f, whatIsGround) 
            || Physics.Raycast(backCheck, out slopeCheck, .5f, whatIsGround) 
    ...
    /*private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(groundCheck.position, .5f);
        Gizmos.DrawWireSphere(transform.position + transform.up * .5f, .5f);

        Debug.DrawRay(groundCheck.position, Vector3.forward * .5f);
        Debug.DrawRay(groundCheck.position, Vector3.back * .5f);
        Debug.DrawRay(groundCheck.position, Vector3.right * .5f);
        Debug.DrawRay(groundCheck.position, Vector3.left * .5f);
        Debug.DrawRay(groundCheck.position, Vector3.down * .5f);
    }*/
}
